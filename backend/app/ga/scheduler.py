...
class GAScheduler:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        stop_reason = "Đã chạy xong max generations"
        generation_run = 0
        ...
        for generation in range(self.ga_cfg.max_generations):
            ...
            # Điều chỉnh giá tị fitness cho việc kiểm tra điều kiện dừng
            scalar_fitness = current_best_fitness
            if isinstance(current_best_fitness, tuple):
                scalar_fitness = (
                    current_best_fitness[0] * 10000 + current_best_fitness[1]
                )

            # Cập nhật cá thể tốt nhất toàn bộ
            ...
            # Thực hiện các toán tử di truyền
