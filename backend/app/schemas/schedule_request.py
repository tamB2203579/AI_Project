...
class FitnessConfig(BaseModel):
    method: str = Field(default="weighted", description="weighted | penalty | alpha_beta | lexicographic")
    ...
    alpha: int = Field(default=1000, description="Trọng số hard constraint for alpha_beta_fitness")
    beta: int = Field(default=10, description="Trọng số soft constraint for alpha_beta_fitness")


class SelectionConfig(BaseModel):
    method: str = Field(default="tournament", description="roulette | tournament | rank | elimination")
    tournament_k: int = Field(default=3, ge=2, description="Top-k for tournament selection")


class CrossoverConfig(BaseModel):
    method: str = Field(default="single_point", description="single_point | two_point | multi_point | uniform")
    rate: float = Field(default=0.8, ge=0.0, le=1.0, description="Tỷ lệ lai ghép")
    n_points: int = Field(default=3, ge=1, description="Số điểm cắt for multi_point crossover")
