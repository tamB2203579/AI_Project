...
import { runGAAsync } from '../ga/geneticAlgorithm';
...
export function GAControls() {
    ...
    const drawChart = useCallback(() => {
        ...
        const history = localHistory.length > 0 ? localHistory : state.fitnessHistory;
        ...
        const lastY = padding.top + chartH - ((history[history.length - 1] - minFit) / range) * chartH;
        ctx.beginPath();
    ...
    return (
        <div className="ga-controls">
            <div className="page-header">
                ...
                <p className="page-description">Configure and run the genetic algorithm to generate an optimal timetable</p>
            </div>
                ...
                <div className="ga-params">
                    ...
                    <div className="param-group">
                        <label className="param-label">
                            Population Size
                            <span className="param-value">{state.gaConfig.populationSize}</span>
                        </label>
                        <input
                            type="range"
                            min={10}
                            ...
                            onChange={e => dispatch({ type: 'SET_GA_CONFIG', payload: { populationSize: Number(e.target.value) } })}
                            className="slider"
                    ...
                    <div className="param-group">
                        <label className="param-label">
                            Mutation Rate
                            <span className="param-value">{(state.gaConfig.mutationRate * 100).toFixed(0)}%</span>
                        </label>
                        <input
                            type="range"
                            min={1}
                            ...
                            onChange={e => dispatch({ type: 'SET_GA_CONFIG', payload: { mutationRate: Number(e.target.value) / 100 } })}
                            className="slider"
                            disabled={state.gaStatus === 'running'}
                        />
                    </div>
                    ...
                    <div className="param-group">
                        <label className="param-label">
                            Crossover Rate
                            <span className="param-value">{(state.gaConfig.crossoverRate * 100).toFixed(0)}%</span>
                        </label>
                        <input
                            type="range"
                            min={50}
                            ...
                            onChange={e => dispatch({ type: 'SET_GA_CONFIG', payload: { crossoverRate: Number(e.target.value) / 100 } })}
                            className="slider"
                            disabled={state.gaStatus === 'running'}
                        />
                    </div>
                    ...
                    <div className="param-group">
                        <label className="param-label">
                            Max Generations
                            <span className="param-value">{state.gaConfig.maxGenerations}</span>
                        </label>
                        <input
                            type="range"
                            min={50}
                            ...
                            onChange={e => dispatch({ type: 'SET_GA_CONFIG', payload: { maxGenerations: Number(e.target.value) } })}
                            className="slider"
                            disabled={state.gaStatus === 'running'}
                        />
                    </div>
                    ...
                    <div className="param-group">
                        <label className="param-label">
                            Tournament Size
                            <span className="param-value">{state.gaConfig.tournamentSize}</span>
                        </label>
                        <input
                            type="range"
                            min={2}
                            ...
                            onChange={e => dispatch({ type: 'SET_GA_CONFIG', payload: { tournamentSize: Number(e.target.value) } })}
                            className="slider"
                            disabled={state.gaStatus === 'running'}
                        />
                    </div>
                    ...
                    <div className="control-buttons">
                        ...
                        {state.gaStatus === 'running' && (
                            <button className="btn-stop" onClick={handleStop} id="btn-stop-ga">
                                ⏹️ Stop
                            </button>
                        )}
                        <button className="btn-reset" onClick={handleReset} id="btn-reset-ga">
                            🔄 Reset
                        </button>
                    </div>
                </div>
                ...
                <div className="ga-chart-section">
                    ...
                    <div className="stats-row">
                        ...
                        <div className="stat">
                            ...
                            <span className="stat-value highlight">{state.bestFitness.toFixed(1)}</span>
                        </div>
                        <div className="stat">
                            <span className="stat-label">Status</span>
                            <span className={`stat-value status-${state.gaStatus}`}>
                                {state.gaStatus === 'running' ? '● Running' : state.gaStatus === 'completed' ? '✓ Done' : '○ Idle'}
                            </span>
                        </div>
                    </div>
